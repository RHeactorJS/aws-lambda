...
export function handler (contentType, environment, tokenSecretOrPrivateKey, operations, event, context, callback) {
  ...
  const done = (err, res) => {
    ...
    if (err && !(err instanceof HttpProblem)) {
      err = new HttpProblem(new URIValue('https://github.com/RHeactorJS/aws-lambda#' + err.constructor.name), err.message, 400)
    }
  ...
  Promise
    .try(() => {
      ...
      if (!operation.length || !operations[operation]) throw new HttpProblem(new URIValue('https://github.com/RHeactorJS/aws-lambda#Error'), `Unknown operation "${event.path}"`, 404)
      if (!allowedMethods.test(event.httpMethod)) {
        throw new HttpProblem(new URIValue('https://github.com/RHeactorJS/aws-lambda#Error'), `Method not allowed: "${event.httpMethod}"`, 405)
      }
      const method = event.httpMethod.toLowerCase()
      if (typeof operations[operation][method] === 'undefined') {
        throw new HttpProblem(new URIValue('https://github.com/RHeactorJS/aws-lambda#Error'), `Unsupported operation "${event.httpMethod} ${event.path}"`, 400)
      }
